...
import ThirdPartyEmailPassword from
...
import {v1pub, v1sec} from './routes';
...
if (process.env.NODE_ENV === 'production' &&
    process.env
    .PRODUCTION) { // only enforce connections to the production server,
        // everything non-production is only guarded through
        // Cloudflare (insecure between CF and Heroku, matter of
        // development infrastructure cost), this serves to
        // encrypt the production traffic between Heroku and
        // Cloudflare
    ...
    const spaceHosts: (string)[] =
        [ process.env.SPACE_HOST, process.env.SPACE_HOST_HEROKU ];
    const apiHosts: (string)[] =
        [ process.env.API_HOST, process.env.API_HOST_HEROKU ];
    supertokens.init({
        framework : 'express',
        supertokens : {connectionURI : connectionUri, apiKey : apiKey},
        appInfo : {
            appName : 'lumium-api',
                            ...
                            return {
                                ...
                                verifyEmailPOST : async function(input) {
                                    ...
                                    if (response.status === 'OK') {
                                        ...
                                        const mails: Email[] =
                                            await dataSource.getRepository(Email).findBy({email});
                                        if (mails?.length == 1) {
                                            ...
                                            await error({
                                                user : {id},
                                                detail,
                                                type : AuditEntryEvent.USER_EMAIL_VERIFICATION_FAILED
                                            });
                                        ...
                                        await info({
                                            user : {id},
                                            detail : email,
                                            type : AuditEntryEvent.USER_EMAIL_VERIFIED
                                        });
                                    }
                                    return response;
                                }
                        ...
                        return {
                                    ...
                                    let user = await dataSource.getRepository(User).findOne(
                                        ...
                                        if (!user) {
                                            user = await dataSource.getRepository(User).save(
                                                {id : response.user.id});
                                                await dataSource.getRepository(Email).save(
                                                    {user, primary : true, email : response.user.email});
                                                    await info({
                                                        user : user,
                                                        type : AuditEntryEvent.USER_SIGNUP_INIT_DEVELOPMENT_PATCH
                                                    });
                                        }
                                }
                            ...
                            emailPasswordSignUp : async function(input) {
                                ...
                                if (response.status === 'OK') {
                                    ...
                                    await error({
                                        detail : JSON.stringify(response) + `email: ${input.email}`,
                                        type : AuditEntryEvent.USER_SIGNUP_FAILED
                                    });
                                }
                                return response;
                            },
    ...
    const windowMs: number =
        Number(process.env.EXPRESS_RATE_LIMIT_WINDOW_MILLISECONDS) || 1000;
