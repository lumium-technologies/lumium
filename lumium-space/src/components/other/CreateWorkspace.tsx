...
import { create_workspace, get_local_storage_password_key, get_space_workspace_id } from "lumium-renderer";
...
export const CreateWorkspace = ({ standalone = true }: CreateWorkspaceProps) => {
    ...
    const handleDownloadKeys = () => {
        create_workspace(formik.values.password, formik.values.name).then((workspace) => {
            localStorage.setItem(get_local_storage_password_key(), formik.values.password);
            Router.push(get_space_workspace_id() + workspace.id);
        });
    };
